...
module.exports = {
    getCart : (req,res) => {
        let sql = `select cartId, userId, v.variantId, priceId, quantity, size, color, image, p.name, p.id, c.name as category, gender, categoryId from cart
                    ...
                    where userId = ${req.query.userId} order by variantId asc`

        // let sql2 = `select * from cart join prices p on p.pricesId = cart.priceId`
    ...
    addToCart : (req, res) => {
        let sql = `insert into cart values (0, '${req.body.userId}', '${req.body.variantId}', '${req.body.pricesId}', '${req.body.quantity}' , '${req.body.size}')`
